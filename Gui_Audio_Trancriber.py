...
client = Groq()
lanng = st.radio("Choose a language of the audio  'en' for Englsih | 'ur' for urdu  ", ["en", "ur"])
...
# Function to list audio files in a folder
def list_audio_files_in_folder(folder_path):
    # List all audio files in the specified folder
    audio_files = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith((".wav", ".mp3", ".m4a"))]
    return audio_files
...
def main():
    ...
    if choice == "Select a Single File":
        ...
        if uploaded_file is not None:
            ...
            # store_choice = st.radio("How would you like to store the transcriptions?", ["Store Simple Trancription", "Store in Kaldi Structure"])
            # store_in_kaldi_format = (store_choice == "Store in Kaldi Structure")
            # Add a "Transcribe" button
            if st.button("Transcribe"):
                transcription_text = transcribe_single_file(file_path)

                # Display audio file name and transcription side by side
                st.write("Audio File Name And Trancriptions ")
        ...
        if folder_path:
            if os.path.isdir(folder_path):
                ...
                if audio_files:
                    st.write(f"Found these following audio files in folder : '{folder_path}'" )
                    ...
                    st.write("No audio files found in the specified folder.")
            else:
